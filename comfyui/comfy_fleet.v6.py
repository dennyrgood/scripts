...
function setModelFilter(f) {{
  modelFilter=f;
  ['all','img','travel','missing'].forEach(id=>document.getElementById('fmod-'+id).classList.toggle('active',id===f));
  renderModels();
}}
...
function wfMatchesFilter(wf) {{
  ...
  if(wfFilter==='tb') return /_tb[_.]|_tb$/.test(n);
  if(wfFilter==='c') return /_c[_.]|_c$/.test(n) && !/_tb/.test(n);
  if(wfFilter==='i') return /_i[_.]|_i$/.test(n);
  return true;
}}
...
function modelMatchesFilter(m) {{
  ...
  const oh=m.on_hosts;
  if(modelFilter==='img') return oh.IMAGEBEAST && !oh.CHATWORKHORSE && !oh.TRAVELBEAST;
...
function renderWf() {{
  ...
  const wfFiltered=filtered; container.innerHTML=filtered.map((wf,i)=>{{
    const isSel=selectedWf===wf.key;
    const isHi=highlightedWfs.has(wf.name||wf.key);
    const isDim=hasSel&&!isHi&&!isSel;
    ...
    return '<div class="'+cls+'" data-idx="'+i+'" onclick="handleWfClick(this)">'
      +dots+'<span class="item-name" title="'+wf.name+'">'+wf.name+'</span>'+meta+prune+'</div>';
  }}).join('');
}}
...
function renderModels() {{
  ...
  const modFiltered=filtered; container.innerHTML=filtered.map((m,i)=>{{
...
<div class="main">
  <div class="panel panel-workflows">
    ...
    <div class="info-bar" id="wf-info">Click a workflow to see its models</div>
  </div>
  <div class="panel panel-models">
    ...
    <div class="filters">
      ...
      <button class="filter-chip" id="fmod-img" onclick="setModelFilter('img')">IMG only</button>
    ...
    <div class="info-bar" id="model-info">Click a model to see its workflows</div>
  </div>
</div>
<script>{JS}</script>
</body>
