# backup-models-bare-to-nas-smb.ps1
# One-time backup: OneDrive Models_bare (ib) -> FleetNAS \\192.168.178.123\misc\models_backup\Models_bare
# Priority 2 (lower priority: already live-shared to tb and cwh separately).
#
...
#
# Pre-check already done: verified 2026-09-18 that all 87 files in this OneDrive
...
#
# Usage:
#   .\backup-models-bare-to-nas-smb.ps1                # dry run, additive only (/L: lists only, copies nothing)
#   .\backup-models-bare-to-nas-smb.ps1 go              # real run, additive only (adds/updates, never deletes)
#   .\backup-models-bare-to-nas-smb.ps1 -Mirror         # dry run, MIRROR (also lists what it would DELETE on the NAS)
#   .\backup-models-bare-to-nas-smb.ps1 go -Mirror      # real run, MIRROR -- DELETES NAS-side files/folders
...
$Src  = "C:\Users\Pc\OneDrive\DropBoxReplacement\MathesDropBox\0ComfyUI\Models_bare"
$Dest = "\\192.168.178.123\misc\models_backup\Models_bare"
$LogDir = "$env:USERPROFILE\.cache\fleetnas-sync"
New-Item -ItemType Directory -Force -Path $LogDir | Out-Null
$Ts = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = Join-Path $LogDir "ib_models_bare_${Ts}.log"
