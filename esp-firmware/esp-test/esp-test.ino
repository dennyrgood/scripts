/*
 * ESP-TEST — minimal sketch, no WiFi/JSON/networking at all.
 * Just displays "THIS IS A TEST / THIS IS ONLY A TEST" centered on screen.
 * Purpose: isolate whether tonight's repeated flash failures are caused by
 * fleet_wall's larger binary/payload, or are a pure connection/hardware
 * issue independent of what's being flashed. Same verified board init as
 * fleet_wall (copied unchanged from Waveshare's official demo).
 */
...
void setup()
{
    ...
    lv_obj_set_style_bg_color(scr, lv_color_hex(0x0a1011), 0);

    lv_obj_t *label = lv_label_create(scr);
    lv_label_set_text(label, "THIS IS A TEST\nTHIS IS ONLY A TEST");
    lv_obj_set_style_text_color(label, lv_color_hex(0x35d6c1), 0);
