...
#define ESP_PANEL_BOARD_EXPANDER_POST_BEGIN_FUNCTION(p) \
    {  \
        ...
        /* esp-functionkeys only: this board's native USB-OTG port shares its \
         * physical connector with a CAN bus interface: EXIO5 low routes it \
         * to USB, EXIO5 high (or left floating) routes it to CAN instead. \
         * Confirmed 2026-09-16 via Waveshare's own FAQ -- without this, the \
         * HID keyboard never enumerates on the Mac at all, regardless of \
         * cable/port, because the port isn't electrically connected to the \
         * S3's native USB pins in the first place. */ \
